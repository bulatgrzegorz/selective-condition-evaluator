...
using ErrorUtils = SelectiveConditionEvaluator.msbuild.Shared.ErrorUtilities;
using InvalidProjectFileException = SelectiveConditionEvaluator.msbuild.Errors.InvalidProjectFileException;
using InvalidToolsetDefinitionException = SelectiveConditionEvaluator.msbuild.Errors.InvalidToolsetDefinitionException;
...
namespace SelectiveConditionEvaluator.msbuild.Definition
{
    ...
    internal abstract class ToolsetReader
    {
            ...
            ToolsetDefinitionLocations locations)
        {
            ...
            if ((locations & ToolsetDefinitionLocations.Registry) == ToolsetDefinitionLocations.Registry)
            {
...
#endif
                {
                    ...
                    var libraryPath = NativeMethods.FrameworkBasePath;
                    ...
                    if (!string.IsNullOrEmpty(libraryPath))
                    {
                        ...
                        if (FileSystems.Default.DirectoryExists(xbuildToolsetsDir))
                        {
                            ...
                            foreach (var d in Directory.GetDirectories(xbuildToolsetsDir).Where(d => r.IsMatch(d)))
                            {
                                ...
                                if (NativeMethods.IsMono && Version.TryParse(version, out Version parsedVersion) && parsedVersion.Major > 14)
                                {
                                    continue;
                                }
            ...
            bool accumulateProperties)
        {
            ...
            try
            {
                var importSearchPathsTable = GetProjectImportSearchPathsTable(toolsVersion.Name, NativeMethods.GetOSNameForExtensionsPath());
