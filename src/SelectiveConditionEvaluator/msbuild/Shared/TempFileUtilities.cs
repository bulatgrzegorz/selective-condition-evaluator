...
namespace SelectiveConditionEvaluator.msbuild.Shared
{
    ...
    internal static partial class FileUtilities
    {
        ...
        private static string CreateFolderUnderTemp()
        {
            ...
            string msbuildTempFolder = msbuild.NativeMethods.IsWindows ?
            ...
            if (msbuild.NativeMethods.IsLinux && msbuild.NativeMethods.mkdir(basePath, userRWX) != 0)
            {
                if (msbuild.NativeMethods.chmod(basePath, userRWX) == 0)
                {
                ...
                else
                {
                    ...
                    while (msbuild.NativeMethods.mkdir(pathToCheck, userRWX) != 0 && msbuild.NativeMethods.chmod(pathToCheck, userRWX) != 0)
                    {
                        extraBits++;
