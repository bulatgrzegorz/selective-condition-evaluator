...
namespace SelectiveConditionEvaluator.msbuild.Shared
{
    ...
    internal static partial class FileUtilities
    {
            ...
            foreach (string path in assemblyPaths)
            {
                if (path != null)
                {
                    ...
                    if (directoryPath.Length > 0)
                    {
                        ...
                        if (msbuild.NativeMethods.GetLastWriteDirectoryUtcTime(directoryPath, out lastModifiedTime))
                        {
        ...
        internal static string MaybeAdjustFilePath(string value, string baseDirectory = "")
        {
            ...
            // have no slashes.
            if (msbuild.NativeMethods.IsWindows || string.IsNullOrEmpty(value)
        ...
        {
            if (msbuild.NativeMethods.IsWindows || value.IsEmpty)
            {
                return value;
            }
        ...
        internal static string GetFullPath(string fileSpec, string currentDirectory)
        {
            ...
            if (msbuild.NativeMethods.IsWindows && !EndsWithSlash(fullPath))
            {
        ...
        private static bool IsPathTooLong(string path)
        {
            ...
            return path.Length >= msbuild.NativeMethods.MaxPath;
        }

        private static bool IsPathTooLongIfRooted(string path)
        {
            bool hasMaxPath = msbuild.NativeMethods.HasMaxPath;
            int maxPath = msbuild.NativeMethods.MaxPath;
            // >= not > because MAX_PATH assumes a trailing null
            return hasMaxPath && !IsRootedNoThrow(path) && msbuild.NativeMethods.GetCurrentDirectory().Length + path.Length + 1 /* slash */ >= maxPath;
        }
