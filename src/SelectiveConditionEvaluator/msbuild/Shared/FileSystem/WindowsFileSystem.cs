...
namespace SelectiveConditionEvaluator.msbuild.Shared.FileSystem
{
    ...
    internal sealed class WindowsFileSystem : ManagedFileSystem
    {
        ...
        {
            return msbuild.NativeMethods.DirectoryExistsWindows(path);
        }

        public override bool FileExists(string path)
        {
            return msbuild.NativeMethods.FileExistsWindows(path);
        }

        public override bool FileOrDirectoryExists(string path)
        {
            return msbuild.NativeMethods.FileOrDirectoryExistsWindows(path);
        }

        public override DateTime GetLastWriteTimeUtc(string path)
        {
            var fileLastWriteTime = msbuild.NativeMethods.GetLastWriteFileUtcTime(path);
            ...
            {
                msbuild.NativeMethods.GetLastWriteDirectoryUtcTime(path, out var directoryLastWriteTime);
