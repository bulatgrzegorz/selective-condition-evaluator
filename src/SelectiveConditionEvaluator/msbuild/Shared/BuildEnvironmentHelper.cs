    ...
    internal sealed class BuildEnvironmentHelper
    {
        ...
        private static string GetMSBuildExeFromVsRoot(string visualStudioRoot)
        {
            return FileUtilities.CombinePaths(
                ...
                CurrentToolsVersion,
                "Bin",
                msbuild.NativeMethods.ProcessorArchitecture == msbuild.NativeMethods.ProcessorArchitectures.X64 ? "amd64" :
                msbuild.NativeMethods.ProcessorArchitecture == msbuild.NativeMethods.ProcessorArchitectures.ARM64 ? "arm64" : string.Empty,
                "MSBuild.exe");
        }
    ...
    internal sealed class BuildEnvironment
    {
        public BuildEnvironment(BuildEnvironmentMode mode, string currentMSBuildExePath, bool runningTests, bool runningInVisualStudio, string visualStudioPath)
        {
            ...
            if (mode == BuildEnvironmentMode.VisualStudio)
            {
                ...
                MSBuildToolsDirectoryRoot = msbuild.NativeMethods.ProcessorArchitecture switch
                {
                    msbuild.NativeMethods.ProcessorArchitectures.X86 => CurrentMSBuildToolsDirectory,
                    msbuild.NativeMethods.ProcessorArchitectures.X64 or msbuild.NativeMethods.ProcessorArchitectures.ARM64
                        => currentToolsDirectory.Parent?.FullName,
                    _ => throw new InternalErrorException("Unknown processor architecture " + msbuild.NativeMethods.ProcessorArchitecture),
                };
            }
            else
            {
