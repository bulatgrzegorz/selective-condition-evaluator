...
using ProjectErrorUtilities = SelectiveConditionEvaluator.msbuild.Shared.ProjectErrorUtilities;
                ...
                switch (action)
                {
                    case (FileMatcher.SearchAction.LogDriveEnumeratingWildcard):
                        switch (loggingMechanism)
                        {
                            ...
                            default:
                                throw new Shared.InternalErrorException($"Logging type {loggingMechanism.GetType()} is not understood by {nameof(GetFileList)}.");
                        }
